...
void utf8_converter(uint32_t utf_32) {
    if (!legal(utf_32)) {
        ...
        putchar(((0xC0 | utf_32>>6)<<8)
            + (0x80 | (utf_32 & 0x3F)));
    } else if (utf_32 <= 0xFFFF) {
        putchar(((0xE0 | utf_32>>12)<<16)
            + ((0x80 | (utf_32>>6 & 0x3F))<<8)
            + (0x80 | (utf_32 & 0x3F)));
    } else {
        putchar(((0xF0 | utf_32>>18)<<24)
            + ((0x80 | utf_32>>12)<<16)
            + ((0x80 | (utf_32>>6 & 0x3F))<<8)
            + (0x80 | (utf_32 & 0x3F)));
    }
...
int utf8_converter_int(uint32_t utf_32) {
    uint32_t utf_8;
    if (!legal(utf_32)) {
        RUN_CODE = -1;
        return 0xFFFD;
    } else if (utf_32 > 0x10FFFF) {
    RUN_CODE = -1;
        return 0x10FFFF;
    } else if(utf_32 <= 0x7F) {
        utf_8 = utf_32 & 0xFF;
    } else if (utf_32 <= 0x7FF) {
        utf_8 = ((0xC0 | utf_32>>6)<<8)
            + (0x80 | (utf_32 & 0x3F));
    } else if (utf_32 <= 0xFFFF) {
        utf_8 = ((0xE0 | utf_32>>12)<<16)
            + ((0x80 | (utf_32>>6 & 0x3F))<<8)
            + (0x80 | (utf_32 & 0x3F));
    } else {
        utf_8 = ((0xF0 | utf_32>>18)<<24)
            + ((0xE0 | utf_32>>12)<<16)
            + ((0x80 | (utf_32>>6 & 0x3F))<<8)
            + (0x80 | (utf_32 & 0x3F));
    }
...
int main() {
    ...
    while (true) {
        ...
        for (int i = 0; i < bytes_read; i++) {
            ...
            printf("\tUTF8: %x \t U+%x\n ", utf8_converter_int(buffer[i]), buffer[i]);
        }
    }
    return RUN_CODE;
}
